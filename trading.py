...
if heure_actuelle < 8 or heure_actuelle >= 18:
    print(f"Hors des horaires de trading actifs ({heure_actuelle}h{minute_actuelle:02d} Paris). Le bot est en veille.")
    exit(0)
...
for ticker in actifs_forex:
    ...
    try:
        ...
        # --- 1. L'INTELLIGENCE PRO : ANTI-CHASING DYNAMIQUE ---
        # Le bot calcule l'écartement du prix par rapport à sa moyenne.
        # Si cet écart est supérieur à 1.8 fois la volatilité (ATR) actuelle de la paire, 
        # le mouvement est épuisé. On refuse d'entrer au pire moment.
        distance_mm20_brute = abs(prix_actuel - sma20_m15)
        limite_sur_extension = atr * 1.8 
        
        if distance_mm20_brute > limite_sur_extension:
            print(f"[{nom_paire}] Rejet : Sur-extension détectée (Mouvement épuisé).")
            continue
        ...
        if signal:
            # --- 3. MONEY MANAGEMENT PRO : SL & TP DYNAMIQUES ---
            sl_pips = round((atr * 1.5) / pip_size, 1)
            # Garde-fou pour éviter les anomalies de données boursières
            sl_pips = max(5.0, min(sl_pips, 35.0))

            is_long = signal == "ACHAT (LONG)"

            if (is_long and tendance_d1_haussiere) or (not is_long and not tendance_d1_haussiere):
                rr = 1.5
            ...
            message = (
                ...
                f"⏱️ **Durée estimée réaliste :** 45 à 90 min\n"
